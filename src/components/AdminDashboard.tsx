...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  const [activeTab, setActiveTab] = useState<
    'overview' | 'api-status' | 'blacklist' | 'server-health' | 'devices' | 'audit' | 'errors' | 'feedbacks' | 'faqs' | 'settings'
  >('overview');
  ...
  const handleToggleMaintenance = (enabled: boolean, minutesOverride?: number) => {
    ...
    if (enabled) {
      ...
      const targetTime = Date.now() + totalMins * 60 * 1000;
  ...
  const handleSaveNotice = (e: React.FormEvent) => {
    ...
    window.dispatchEvent(new Event('savetik-maintenance-changed'));
    alert('Judul, Badge & Pesan Pengumuman Perbaikan Berhasil Diperbarui!');
  };
