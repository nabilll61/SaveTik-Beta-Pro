    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <section className="mb-12">
        ...
        <div className="space-y-5">
          ...
          {faqs.map((faq, index) => (
            <div key={index} className="bg-neo-card p-5 border-[3px] border-neo-border rounded-[10px] shadow-[4px_4px_0px_0px_#F97316] transition-colors">
              <h4 className="font-heading font-black text-sm md:text-base text-[#D97706] mb-2 flex items-center gap-2">
                <HelpCircle size={18} className="text-[#D97706] shrink-0 stroke-[2.5]" />
                <span>{faq.question}</span>
              </h4>
              <p className="text-xs md:text-sm text-neo-text opacity-70 font-semibold leading-relaxed pl-6">
                {faq.answer}
              </p>
            </div>
          ))}
