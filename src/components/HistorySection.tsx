...
export default function HistorySection({ onBack, onSelectVideo, showToast }: HistorySectionProps) {
  ...
  useEffect(() => {
    ...
    const cleaned = saved.filter(item => {
      ...
      const authorId = (item.authorUniqueId || '').toLowerCase();
      if (
        title.includes('duka - spotify artist') || 
        authorId.includes('spotify_music') || 
        authorId.includes('capcut_template')
      ) {
        return false;
      }
      return true;
    }).map(item => {
      if (item.title === 'Instagram Media' || item.caption === 'Instagram Media' || (item.title && item.title.toLowerCase().includes('video extracted from')) || (item.caption && item.caption.toLowerCase().includes('video extracted from'))) {
        modified = true;
    ...
    setHistory(cleaned);
    if (cleaned.length !== saved.length || modified) {
      safeLocalStorage.setJSON('savetik_history', cleaned);
    }
  }, []);
              ...
              >
                ...
                <div className="p-4 flex-grow flex flex-col justify-between">
                  <div>
                    ...
                    {((video.authorUniqueId && !video.authorUniqueId.includes('capcut_template') && !video.authorUniqueId.includes('spotify_music')) || (video.author && !video.author.includes('Spotify Artist'))) && (
