        ...
        if (response.data && response.data.code === 0 && response.data.data) {
          ...
          if (resData.images && Array.isArray(resData.images)) {
            ...
            if (wmVideoUrl) {
              ...
              directLinks.push({
                quality: 'Video With Watermark HD',
                format: 'mp4',
                size: '',
      ...
      try {
        ...
        if (capcutRes && capcutRes.data) {
          ...
          if (capData.videos && capData.videos.length > 0) {
            ...
            directLinks.push({
              quality: 'Video CapCut Dengan Watermark HD',
              format: v.suffix || 'mp4',
              size: v.size || '',
          ...
          if (capData.audios && capData.audios.length > 0) {
            capData.audios.forEach((a, idx) => {
              const audioLabel = a.size ? `Audio CapCut ${a.size}` : `Audio Sound CapCut ${idx + 1}`;
              directLinks.push({
                quality: audioLabel,
                format: a.suffix || 'mp3',
                size: a.size || '',
