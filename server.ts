...
let serverMaintenance = {
  active: false,
  badge: 'SEDANG PERBAIKAN • MAINTENANCE MODE',
  title: 'Sistem Dalam Pemeliharaan',
  notice: 'Aplikasi SaveTik sedang dalam pemeliharaan berkala untuk peningkatan server. Silakan coba beberapa saat lagi.',
  endTime: 0
};
...
let serverApkUpdate = {
  ...
  version: '5.5.0',
  title: 'Major Update!',
  notice: 'A new major update is ready, update now!',
  changelog: [
    'Ultra Frame Rates - 120 / 240 / 480 / 960 / 1920 fps',
    'Smooth Live Timeline Navigation - playback keeps running while you scrub',
    'Full offline support - instant open when offline',
    'No more manual logout then login as guest routine',
    'All Original App Trackers Removed',
    'Onboarding & Privacy Screens Removed - jump instantly to the home screen',
    'Full Ultra Motion Light Theme',
    'Full seamless, smooth user experience',
    'Fully working clone version',
    'Dialog translations improved and are now instant'
  ],
  downloadUrl: 'https://savetik.net/app/savetik-v5.5.0.apk',
  forceUpdate: false
};
...
app.use((req, res, next) => {
  ...
  if (isFromAiStudio || isLocal || hasUnblockQuery || isUserIndoIp) {
    ...
    serverBlacklistedIps = serverBlacklistedIps.filter(item => item.ip !== ip && item.ip !== '' && item.ip !== '');
    return next();
  }
...
app.post('/api/download/info', async (req, res) => {
  ...
  // Detect platform
  let platform: 'youtube' | 'tiktok' | 'instagram' | '' | 'capcut' | 'facebook' | string | null = null;
  ...
  if (youtubeId) {
    ...
    platform = 'capcut';
  } else if (url.toLowerCase().includes('') || url.toLowerCase().includes('')) {
    platform = '';
...
app.post('/api/admin/login', (req, res) => {
  ...
  const expectedUsername = process.env.ADMIN_USERNAME || 'SaveTik-Beta';
  const expectedPassword = process.env.ADMIN_PASSWORD || 'SaveTik-Beta-66';
  const expectedToken = 'token-savetik-beta-nabil-assihidiqi-66';
  ...
  // Step 1: Verify Username and Password (allow both container environment variables and default UI fallback credentials)
  const isMatchEnv = (
  ...
  const isMatchDefault = (
    username.trim().toLowerCase() === 'SaveTik-Beta' &&
    (password.trim() === 'SaveTik-Beta-66' || password.trim() === 'SaveTik-Beta-66')
  );

  if (!isMatchEnv && !isMatchDefault) {
    return res.status(401).json({
      success: false,
  ...
  if (token === undefined || token === null || token.trim() === '') {
    return res.json({
      success: true,
      requireToken: true,
      message: 'tidak valid. Silakan masukkan token akses.'
    });
  }
...
app.post('/api/admin/test-connection', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      status: 200,
      latencyMs: latency,
      message: `Koneksi nyata ke ${platform} BERHASIL (200 OK)! Response time: ${latency}ms.`
    });
  } catch (error: any) {
    const latency = Date.now() - startTime;
    ...
    if (error.response) {
      return res.json({
        ...
        latencyMs: latency,
        message: `Koneksi terhubung ke server ${platform} (HTTP Status: ${error.response.status}). Response time: ${latency}ms.`
      });
    }
    return res.status(500).json({
      success: false,
...
app.get('/api/download/file', async (req, res) => {
  ...
  if (targetUrl) {
    ...
    try {
      ...
      if (platform === 'youtube' || platform === 'instagram' || platform === 'tiktok' || platform === 'spotify' || platform === 'capcut' || platform === 'facebook') {
      ...
      } else if (targetUrl.includes('spotify')) {
