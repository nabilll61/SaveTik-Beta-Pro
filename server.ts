...
app.post('/api/download/info', async (req, res) => {
  ...
  if (platform === 'youtube') {
    return res.status(200).json({
      success: false,
      message: "Mohon maaf, server downloader YouTube sedikit ada gangguan untuk sementara ini. Silakan gunakan dulu pengunduhan TikTok, Instagram, Facebook, CapCut, yang tetap aktif & normal terimakasih :)"
    });
  }

  if (!platform) {
    return res.status(400).json({ success: false, message: "URL tidak dikenali! Mendukung link TikTok, Instagram, Facebook, CapCut,." });
  }
      ...
      try {
        ...
        if (capcutRes && capcutRes.data) {
          ...
          if (capData.videos && capData.videos.length > 0) {
            ...
            const qualityLabelNoWm = v.size ? `No Watermark ${v.size}` : `No Watermark HD`;
            directLinks.push({
          ...
          if (capData.audios && capData.audios.length > 0) {
            capData.audios.forEach((a, idx) => {
              const audioLabel = a.size ? `Audio ${a.size}` : `Audio ${idx + 1}`;
              directLinks.push({
                quality: audioLabel,
                format: a.suffix || 'mp3',
                size: a.size || 'Beta',
...
app.get('/api/download/file', async (req, res) => {
  ...
  if (targetUrl) {
    ...
    if (targetUrl.includes('spotify.com') || targetUrl.includes('open.spotify')) {
      ...
      if (!resolvedOk || resolvedUrl.includes('spotify.com')) {
        resolvedUrl = 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3';
      }
    }
    ...
    try {
      ...
      console.warn("Download Proxy Fallback triggered:", err.message);
      // Fall through to fallback file generator
    }
  }
  ...
  if (format === 'mp3') {
    ...
    try {
      const sampleResp = await axios.get('https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3', {
        responseType: 'stream',
        timeout: 10000
      });
      sampleResp.data.pipe(res);
      return;
    } catch (e) {
