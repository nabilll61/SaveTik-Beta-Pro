...
const initFirebase = async () => {
  try {
    ...
    // Helper for timeout to prevent hangs on metadata query when credentials aren't set (e.g. Railway)
    const withTimeout = (promise: Promise<any>, ms: number, label: string) => {
      ...
      return Promise.race([
        ...
        // If the timeoutPromise rejects first, safePromise is still active in background.
        // We attach a dummy catch to handle its eventual rejection and prevent unhandled promise crash.
        safePromise.catch(() => {});
        throw err;
      });
    };
...
async function logIpRequest(req: express.Request, platform: string, status: string, latencyMs?: number) {
  ...
  if (db && platform !== 'system' && platform !== 'admin-test') {
    try {
...
app.get('/api/admin/dashboard-data', async (req, res) => {
  ...
  try {
    ...
    // 2. Get stats from Firestore
    let analytics: any = { totalHits: 0, platforms: {}, os: {}, browsers: {}, regions: {} };
    let dailyHits = 0;
    if (db) {
      try {
        const statsRef = db.collection('analytics').doc('global_stats');
        ...
        if (statsDoc.exists) {
          analytics = statsDoc.data();
        } else {
          // Silently seed the document if it's missing
          await statsRef.set(analytics).catch(() => {});
        }
        ...
        if (dailyDoc.exists) {
          dailyHits = dailyDoc.data()?.hits || 0;
        } else {
          // Seed daily hits if missing
          await dailyRef.set({ hits: 0 }).catch(() => {});
        }
      } catch (dbErr: any) {
        // If it's a 5 NOT_FOUND, it might mean the database or collection doesn't exist yet
        if (dbErr.code === 5 || String(dbErr).includes('NOT_FOUND')) {
          console.warn("Firestore resource not found (Status 5), using empty stats.");
        } else {
